  ...
  try {
    const handler = auth0.withApiAuthRequired(async () => {
      try {
        ...
        console.error('Proxy GET /api/proxy/appointments (inner) failed:', e);
    ...
    console.error('Proxy GET /api/proxy/appointments (outer) failed:', e);
  ...
  try {
    const handler = auth0.withApiAuthRequired(async () => {
      try {
        ...
        console.error('Proxy POST /api/proxy/appointments (inner) failed:', e);
        return new Response(JSON.stringify({ message: e?.message || 'Proxy error' }), {
          status: 502,
          headers: { 'content-type': 'application/json' },
        });
    ...
    console.error('Proxy POST /api/proxy/appointments (outer) failed:', e);
    const status = e?.status || e?.statusCode || 401;
    return new Response(JSON.stringify({ message: e?.message || 'Unauthorized' }), {
      status,
      headers: { 'content-type': 'application/json' },

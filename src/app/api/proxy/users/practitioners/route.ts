  ...
  try {
    const handler = auth0.withApiAuthRequired(async () => {
      try {
        ...
        const res = await fetch(`${API_BASE}/api/users/practitioners`, {
        ...
        console.error('Proxy GET /api/proxy/users/practitioners (inner) failed:', e);
    ...
    console.error('Proxy GET /api/proxy/users/practitioners (outer) failed:', e);
